 ...
 */ function point_geometry_Point(x, y) {
    this.x = x;
    this.y = y;
}
point_geometry_Point.prototype = {
    /**
     * Clone this point, returning a new point that can be modified
     * without affecting the old one.
     * @return {Point} the clone
     */ clone () {
        return new point_geometry_Point(this.x, this.y);
    },
    /**
    ...
    constructor: point_geometry_Point
};
/**
 ...
 */ point_geometry_Point.convert = function(p) {
    if (p instanceof point_geometry_Point) return /** @type {Point} */ p;
    if (Array.isArray(p)) return new point_geometry_Point(+p[0], +p[1]);
    if (void 0 !== p.x && void 0 !== p.y) return new point_geometry_Point(+p.x, +p.y);
...
/**
    ...
    for(let i = 1, l = vertexes.length; i < l; i++){
        current = new point_geometry_Point(vertexes[i].x, vertexes[i].y);
        prev = new point_geometry_Point(vertexes[i - 1].x, vertexes[i - 1].y);
        ...
        next = i < l - 1 ? new point_geometry_Point(vertexes[i + 1].x, vertexes[i + 1].y) : null;
...
const PlotUtils_paintSmoothLine = (ctx, points, lineOpacity, smoothValue, draw, close, tailIdx, tailRatio)=>{
    ...
    for(let i = 0; i < l; i++){
        ...
        if (i === l - 1 && tailRatio >= 0 && tailRatio < 1) {
            ...
            points.splice(l - 1, count - (l - 1) - 1);
            const lastPoint = new point_geometry_Point(ctrlPoints[4], ctrlPoints[5]);
            lastPoint.prevCtrlPoint = new point_geometry_Point(ctrlPoints[2], ctrlPoints[3]);
            points.push(lastPoint);
            count = points.length;
...
class StraightArrow extends maptalks_es_Curve {
    ...
    _getArrowHeadPoint(h1, h2, vertex, lineWidth, f, hScale) {
        if (!hScale) hScale = 1;
        h1 = new point_geometry_Point(h1.x, h1.y);
        h2 = new point_geometry_Point(h2.x, h2.y);
...
class DoveTailDiagonalArrow extends DiagonalArrow {
    ...
    _closeArrow(ctx, last, first) {
        ...
        const t1 = new point_geometry_Point(last.x, last.y);
        const t2 = new point_geometry_Point(first.x, first.y);
        const m = new point_geometry_Point(t1.x + t2.x, t1.y + t2.y).mult(0.5);
